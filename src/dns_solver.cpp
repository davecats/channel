...
void DnsLinearStep::assemble_velocity_yline_coefficients(int component,
  ...
  implicit_yline_->assemble_velocity_coefficients(component,
                                                  k2,
                                                  derivatives,
...
void DnsLinearStep::copy_yline_boundary_data_from_host(int component, const DnsYLineBoundaryData& boundary) {
  if (!implicit_yline_) {
    throw std::runtime_error("DnsLinearStep has no implicit y-line stage");
  }
  implicit_yline_->copy_boundary_data_from_host(component, boundary);
}
...
void DnsLinearStep::advance(DnsState& state) {
  ...
  {
    Kokkos::Profiling::ScopedRegion region("linsolve forward_fft");
    for (auto& plan : forward_ffts_) {
      plan->execute(state, FftDirection::Forward, "dns_linear_forward_fft");
    }
  }
  {
    Kokkos::Profiling::ScopedRegion region("linsolve transpose_to_y");
    for (auto& plan : transposes_) {
      plan->execute(state);
    }
  }
  if (implicit_yline_) {
    Kokkos::Profiling::ScopedRegion region("linsolve implicit_yline");
    implicit_yline_->solve(state);
  }
  {
    Kokkos::Profiling::ScopedRegion region("linsolve mean_correction");
    for (auto& step : mean_corrections_) {
      step->apply(state);
    }
  }
  {
    Kokkos::Profiling::ScopedRegion region("linsolve inverse_fft");
...
void DnsVelocityStep::advance(DnsState& state) {
  ...
  if (nonlinear_product_transform_) {
    Kokkos::Profiling::ScopedRegion region("transform_to_physical_and_build_products");
    nonlinear_product_transform_->apply(state);
  }
  if (nonlinear_velocity_rhs_) {
    Kokkos::Profiling::ScopedRegion region("transform_back_and_build_rhs");
  ...
  if (velocity_recovery_) {
    Kokkos::Profiling::ScopedRegion region("velocity_recovery");
