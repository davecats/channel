...
void SchurLevel::exchange_rows() {
  if (cfg_.exchange_mode == ExchangeMode::AllGather) {
    allgather_complex_device(row_send_.view(), row_width * cfg_.prev_count, row_recv_.view(), cfg_.comm,
                             "schur_rows_allgather");
    return;
  }
  alltoallv_complex_device(row_send_.view(), row_send_counts_, row_send_displs_, row_recv_.view(), row_recv_counts_,
...
void SchurLevel::exchange_values() {
  ...
  alltoallv_complex_device(value_send_.view(), value_send_counts_, value_send_displs_, value_recv_.view(),
...
void SchurSolver::solve_from_leaf_rows(const DeviceVector<Complex>& leaf_rows, DeviceVector<Complex>& leaf_values) {
  ...
  for (std::size_t ilevel = 0; ilevel < levels_.size(); ++ilevel) {
    ...
    {
      Kokkos::Profiling::ScopedRegion region("schur pack_rows");
      level.pack_rows_for_exchange(*current_rows);
    }
    {
      Kokkos::Profiling::ScopedRegion region("schur exchange_rows");
      level.exchange_rows();
    }
    {
      Kokkos::Profiling::ScopedRegion region("schur compose_level");
  ...
  {
    Kokkos::Profiling::ScopedRegion region("schur seed_root_values");
  ...
  for (std::size_t remaining = levels_.size(); remaining > 0; --remaining) {
    ...
    if (redundant_root && ilevel + 1 == levels_.size()) {
      Kokkos::Profiling::ScopedRegion region("schur recover_redundant_root_values");
      level.recover_redundant_root_values(child_values);
      continue;
    }
    {
      Kokkos::Profiling::ScopedRegion region("schur pack_recovered_values");
      level.pack_recovered_values();
    }
    {
      Kokkos::Profiling::ScopedRegion region("schur exchange_values");
      level.exchange_values();
    }
    {
      Kokkos::Profiling::ScopedRegion region("schur unpack_parent_values");
