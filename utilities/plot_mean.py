...
parser.add_argument('--average', '-a', help='If a list of files is provided, computes and plot the ensemble average.', action='store_true')
...
if len(fields) == 1:
    ...
    np, y, _, _ = read_sim_prop(fields[0], dirname(fields[0])+'dns.in')
    mean = numpy.zeros((np[1] + 1))
    for fname in fields:
        v_field = field_as_memmap(fname)
        mean += v_field[0,0,np[2],1:-1].real
    mean /= len(fields)
    fig, ax = plt.subplots()
    ax.plot(y, mean)
