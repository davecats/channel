...
channel::Complex leaf_row_value(int child, int line, int k) {
  const double base = 0.05 * static_cast<double>(child + 1) + 0.012 * static_cast<double>(line + 1);
  if (k < 4) {
    return channel::Complex(1.0 + base + 0.017 * static_cast<double>(k + 1),
                            0.002 * static_cast<double>((child + 1) * (k + 2)));
  }
  return channel::Complex(0.012 + 0.0015 * static_cast<double>(k) + 0.0007 * static_cast<double>(line),
                          -0.0004 * static_cast<double>((child + 2) * (k + 1)));
}
...
std::vector<channel::Complex> expected_leaf_values(int child_id, int arity, int nlines) {
  ...
  for (int line = 0; line < nlines; ++line) {
    ...
    expected[static_cast<std::size_t>(line_value(line, 4))] =
        child_id > 0 ? rhs[static_cast<std::size_t>(rhs_index(row0 - 2, 0))] : channel::Complex(0.0, 0.0);
    expected[static_cast<std::size_t>(line_value(line, 5))] =
        child_id > 0 ? rhs[static_cast<std::size_t>(rhs_index(row0 - 1, 0))] : channel::Complex(0.0, 0.0);
...
int main(int argc, char** argv) {
  ...
  channel::test::require(runtime.size() == 4 || runtime.size() == 6,
                         "Schur solver multilevel test must run with four or six MPI ranks");
  const bool force_root_alltoall = argc > 1 && std::string(argv[1]) == "root-alltoall";
  channel::test::require(!force_root_alltoall || runtime.size() == 6,
                         "root-alltoall Schur multilevel variant requires six MPI ranks");
  ...
  channel::DeviceVector<channel::Complex> leaf_rows_device("schur_multilevel_leaf_rows", leaf_rows.size());
  channel::DeviceVector<channel::Complex> leaf_values_device("schur_multilevel_leaf_values", nlines * value_width);
  ...
  channel::test::require(levels.size() == 2, "multilevel solver has two levels");
  channel::test::require(levels[0].config().exchange_mode == channel::ExchangeMode::AllToAll,
                         "level 0 auto exchange is Alltoallv");
  channel::test::require(levels[0].config().arity == cfg.pass_counts[0], "level 0 arity matches pass sequence");
  channel::test::require(levels[1].config().arity == cfg.pass_counts[1], "level 1 arity matches pass sequence");
  const auto expected_root_exchange =
      force_root_alltoall ? channel::ExchangeMode::AllToAll : channel::ExchangeMode::AllGather;
  channel::test::require(levels[1].config().exchange_mode == expected_root_exchange,
                         force_root_alltoall ? "level 1 auto exchange is root Alltoallv"
                                             : "level 1 auto exchange is root AllGather");
  ...
  for (std::size_t i = 0; i < actual.size(); ++i) {
    channel::test::require_near(actual[i], expected[i], 5.0e-12,
                                "Schur solver multilevel leaf value rank " + std::to_string(runtime.rank()) +
                                    " index " + std::to_string(i));
  }

  if (runtime.rank() == 0) std::cout << "Schur solver multilevel test PASSED\n";
  return 0;
}
