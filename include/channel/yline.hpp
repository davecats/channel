...
struct DnsYLineComponentConfig {
  ...
  ExchangeMode exchange_mode = ExchangeMode::Auto;
  MpiComm comm_y = world_comm();
  bool fill_interface_ghosts = false;
};

struct DnsYLineBoundaryData {
  bool lower_enabled = false;
  bool upper_enabled = false;
  std::array<double, 5> lower_ghost_eq{};
  std::array<double, 5> lower_boundary_eq{};
  std::array<double, 5> upper_boundary_eq{};
  std::array<double, 5> upper_ghost_eq{};
  std::span<const Complex> lower_ghost_rhs{};
  std::span<const Complex> lower_boundary_rhs{};
  std::span<const Complex> upper_boundary_rhs{};
  std::span<const Complex> upper_ghost_rhs{};
};

struct DnsYLineDeviceBoundaryData {
  bool lower_enabled = false;
  bool upper_enabled = false;
  std::array<double, 5> lower_ghost_eq{};
  std::array<double, 5> lower_boundary_eq{};
  std::array<double, 5> upper_boundary_eq{};
  std::array<double, 5> upper_ghost_eq{};
  const DeviceVector<Complex>* lower_ghost_rhs = nullptr;
  const DeviceVector<Complex>* lower_boundary_rhs = nullptr;
  const DeviceVector<Complex>* upper_boundary_rhs = nullptr;
  const DeviceVector<Complex>* upper_ghost_rhs = nullptr;
};
...
public:
  ...
  void assemble_velocity_coefficients(const DeviceVector<double>& k2,
                                      ...
                                      double viscosity,
                                      bool biharmonic);
  void copy_boundary_data_from_host(const DnsYLineBoundaryData& boundary);
  void copy_boundary_data_from_device(const DnsYLineDeviceBoundaryData& boundary);
  void solve(DnsState& state);
  void eliminate_boundaries();
  void reconstruct_boundaries(DnsState& state);
  void exchange_interface_ghosts(DnsState& state);
...
private:
  ...
  DeviceVector<Complex> du_;
  DeviceVector<Complex> dw_;
  DeviceVector<Complex> work_ds_;
  DeviceVector<Complex> work_dl_;
  DeviceVector<Complex> work_d_;
  DeviceVector<Complex> work_du_;
  DeviceVector<Complex> work_dw_;
  DeviceVector<Complex> rhs_;
  DeviceVector<Complex> interface_send_;
  DeviceVector<Complex> interface_recv_;
  bool lower_boundary_enabled_ = false;
  bool upper_boundary_enabled_ = false;
  Kokkos::Array<double, 5> lower_ghost_eq_{};
  Kokkos::Array<double, 5> lower_boundary_eq_{};
  Kokkos::Array<double, 5> upper_boundary_eq_{};
  Kokkos::Array<double, 5> upper_ghost_eq_{};
  DeviceVector<Complex> lower_ghost_rhs_;
  DeviceVector<Complex> lower_boundary_rhs_;
  DeviceVector<Complex> upper_boundary_rhs_;
  DeviceVector<Complex> upper_ghost_rhs_;
};
...
public:
  ...
  void copy_coefficients_from_host(int component,
                                   ...
                                   std::span<const Complex> du,
                                   std::span<const Complex> dw);
  void assemble_velocity_coefficients(int component,
                                      ...
                                      double viscosity,
                                      bool biharmonic);
  void copy_boundary_data_from_host(int component, const DnsYLineBoundaryData& boundary);
  void solve(DnsState& state);
