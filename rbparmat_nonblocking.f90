...
SUBROUTINE LU5decompStep(A,Rs)
  ...
  ELSE 
    CALL MPI_IRecv(buf,10,MPI_DOUBLE_PRECISION,ipy+1,TAG_LUDECOMP,MPI_COMM_Y,Rr)
    CALL MPI_Wait(Rr,S)
    A(HI1-1:HI1,:)=buf
  END IF
  DO i=HI1-HI2,0,-1 
...
!------------------------step 1-----------------------!
SUBROUTINE LeftLU5divStep1(x,A,b,Rs)
  ...
  x=b
  IF (.NOT. last) THEN 
    CALL MPI_IRecv(x(HI1-1:HI1),2,cmpl,ipy+1,TAG_LUDIVSTEP1,MPI_COMM_Y,Rr)
    CALL MPI_Wait(Rr,S)
  END IF
  DO i=HI1-HI2,0,-1 
...
!------- Left LU division of a banded matrix ---------!
!------------------------step 2-----------------------!
SUBROUTINE LeftLU5divStep2(A,b,Rs)
  ...
  HI1=SIZE(A,1)-1; HI2=SIZE(A,2)-3;
  IF (.NOT. first) THEN 
    CALL MPI_IRecv(b(-2:-1),2,cmpl,ipy-1,TAG_LUDIVSTEP2,MPI_COMM_Y,Rr)
    CALL MPI_Wait(Rr,S)
  END IF
  DO i=0,HI1
