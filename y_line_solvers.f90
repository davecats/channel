...
module y_line_solvers
  ...
  public :: ys_applybc_0, ys_applybc_n
  public :: ys_lu5decomp, ys_leftlu5backsub, ys_leftlu5forwardsub, ys_leftlu5div
  public :: ys_solve_compact_derivative, ys_solve_compact_system, ys_solve_ghost_system

contains

  pure subroutine ys_applybc_0(eq, bc0, bc0m1, ny0, nyN)
    integer(C_INT), intent(in) :: ny0, nyN
  ...
  end subroutine ys_applybc_0

  pure subroutine ys_applybc_n(eq, bcn, bcnp1, ny, ny0, nyN)
  ...
  subroutine ys_leftlu5backsub(x, a, b)
  ...
  end subroutine ys_leftlu5backsub

  subroutine ys_leftlu5forwardsub(x, a)
  ...
  end subroutine ys_leftlu5forwardsub

  subroutine ys_leftlu5div(x, a, b)
    complex(C_DOUBLE_COMPLEX), intent(inout) :: x(-2:)
    complex(C_DOUBLE_COMPLEX), intent(in) :: b(-2:)
    real(C_DOUBLE), intent(in) :: a(0:, -2:)
    complex(C_DOUBLE_COMPLEX) :: rhs(lbound(x, 1):ubound(x, 1))

    rhs = b
    call ys_leftlu5backsub(x, a, rhs)
  ...
  subroutine ys_solve_compact_derivative(f0, f1, der, d0mat, d140, d14m1, d14n, d14np1, ny, ny0, nyN)
    ...
    call ys_leftlu5div(f1, d0mat, f1)
  ...
  subroutine ys_solve_compact_system(x, a, lower_bc, lower_ghost_bc, upper_bc, upper_ghost_bc, &
    ...
    call ys_applybc_0(a, lower_bc, lower_ghost_bc, ny0, nyN)
    x(1) = x(1) - a(1, -2)*rhs_lower_ghost/lower_ghost_bc(-2) - a(1, -1)*rhs_lower/lower_bc(-1)
    x(2) = x(2) - a(2, -2)*rhs_lower/lower_bc(-1)

    call ys_applybc_n(a, upper_bc, upper_ghost_bc, ny, ny0, nyN)
    x(ny - 1) = x(ny - 1) - a(ny - 1, 2)*rhs_upper_ghost/upper_ghost_bc(2) - a(ny - 1, 1)*rhs_upper/upper_bc(1)
    x(ny - 2) = x(ny - 2) - a(ny - 2, 2)*rhs_upper/upper_bc(1)

    call ys_lu5decomp(a)
    call ys_leftlu5div(x, a, x)

    x(0) = (rhs_lower - sum(x(1:3)*lower_bc(0:2)))/lower_bc(-1)
    x(-1) = (rhs_lower_ghost - sum(x(0:3)*lower_ghost_bc(-1:2)))/lower_ghost_bc(-2)
    x(ny) = (rhs_upper - sum(x(ny - 3:ny - 1)*upper_bc(-2:0)))/upper_bc(1)
    x(ny + 1) = (rhs_upper_ghost - sum(x(ny - 3:ny)*upper_ghost_bc(-2:1)))/upper_ghost_bc(2)
